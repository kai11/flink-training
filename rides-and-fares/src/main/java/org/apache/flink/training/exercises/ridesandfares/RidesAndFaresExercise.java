...
public class RidesAndFaresExercise {
    ...
    public static class EnrichmentFunction
    	...
    	private transient ValueState<TaxiRide> rides = null;
    	private transient ValueState<TaxiFare> fares = null;
    	
        @Override
        public void open(Configuration config) throws Exception {
        	 ValueStateDescriptor<TaxiRide> ridesDescriptor =
                     new ValueStateDescriptor<>("rides", TypeInformation.of(new TypeHint<TaxiRide>() {}));
             this.rides = getRuntimeContext().getState(ridesDescriptor);
        	 ValueStateDescriptor<TaxiFare> faresDescriptor =
                     new ValueStateDescriptor<>("fares", TypeInformation.of(new TypeHint<TaxiFare>() {}));
             this.fares = getRuntimeContext().getState(faresDescriptor);
        }

        private void addRideAndFareIfBothAvailable(Collector<RideAndFare> out) {
        	try {
				if (this.rides.value() != null && this.fares.value() != null) {
					out.collect(new RideAndFare(this.rides.value(), this.fares.value()));
					this.rides.clear();
					this.fares.clear();
				}
			} catch (IOException e) {
        ...
        public void flatMap1(TaxiRide ride, Collector<RideAndFare> out) throws Exception {
        	this.rides.update(ride);
        	this.addRideAndFareIfBothAvailable(out);
        }

		@Override
        public void flatMap2(TaxiFare fare, Collector<RideAndFare> out) throws Exception {
        	this.fares.update(fare);
        	this.addRideAndFareIfBothAvailable(out);    	
        }
    }
}
